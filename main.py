...
def _quadratic_multiply(x, y):
  ...
  l_halves = bit_shift(_quadratic_multiply(x_left, y_left), len(xvec))
  cross_sum_1 = bit_shift(_quadratic_multiply(x_left, y_right), len(xvec) // 2)
  cross_sum_2 = bit_shift(_quadratic_multiply(x_right, y_left), len(xvec) // 2)
  r_halves = _quadratic_multiply(x_right, y_right)

  return BinaryNumber(l_halves.decimal_val + cross_sum_1.decimal_val +
                      cross_sum_2.decimal_val + r_halves.decimal_val)


def test_quadratic_multiply():
  start = time.time()
  # multiply two numbers x, y using function f
  print(_quadratic_multiply(BinaryNumber(10), BinaryNumber(5)))
